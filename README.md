...
Встроенный набор (если `basemaps` не задан) — тот же, что в mrr2h3:
OpenStreetMap и его немецкое зеркало, **Яндекс** (схема, спутник), **2ГИС**,
**Google** (схема, спутник, гибрид), Carto (светлая, тёмная), Esri (спутник,
улицы, топография), OpenTopoMap, CyclOSM.

**Автопереключение.** Если сервер тайлов не отвечает или блокирует запросы,
страница после нескольких ошибок сама переключается на следующую подложку из
списка и пишет об этом в статусе; под списком остаётся пояснение, почему
подложка не та. Подложку, выбранную человеком вручную, страница не подменяет —
только сообщает, что тайлы не грузятся. Раз в полминуты проверяется, жив ли
текущий сервер тайлов.
